...
export class NanoBananaProvider implements ImageGenerator {
  ...
  constructor(options?: { apiKey?: string; model?: string }) {
    ...
    if (!apiKey) throw new Error("GEMINI_API_KEY is required to activate Nano Banana");
    this.ai = new GoogleGenAI({ apiKey });
    this.model = options?.model ?? process.env.NANO_BANANA_MODEL ?? "gemini-3.1-flash-image-preview";
  }
